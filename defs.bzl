...
def _typescript_proto_library_impl(ctx):
  ...
  protoc_command = "%s --plugin=protoc-gen-ts=%s --ts_out=service=true:%s%s --js_out=import_style=commonjs,binary:%s --descriptor_set_in=%s %s" % (ctx.file._protoc.path, ctx.files._ts_protoc_gen[1].path, ts_out, ctx.var["BINDIR"], ctx.var["BINDIR"], ":".join(descriptor_sets), " ".join(proto_inputs))
